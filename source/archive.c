...
Result open_extdata()
{
	...
	{
		extdata_archives[pos].archive_id = ARCHIVE_SHARED_EXTDATA;

		extdata_archives[pos].archive_path.type = PATH_BINARY;
		extdata_archives[pos].archive_path.size = 0xc;
		extdata_archives[pos].archive_path.data = (u8*)extdata_archives_lowpathdata[pos];

		memset(extdata_archives_lowpathdata[pos], 0, 0xc);
		extdata_archives_lowpathdata[pos][0] = SYSTEM_MEDIATYPE_CTR_NAND;
	}

	extdata_archives_lowpathdata[GameCoin_Extdata][1] = extdataID_gamecoin;//extdataID-low

	ret = FSUSER_OpenArchive(&extdata_archives[GameCoin_Extdata].archive_file, extdata_archives[GameCoin_Extdata].archive_id, extdata_archives[GameCoin_Extdata].archive_path);
	if(ret!=0)
	{
...
void close_extdata()
{
	...
	for(pos=0; pos<TotalExtdataArchives; pos++)
	{
		if(extdata_initialized & (1<<pos))FSUSER_CloseArchive(extdata_archives[GameCoin_Extdata].archive_file);
	}
}
...
Result archive_getfilesize(Archive archive, char *path, u32 *outsize)
{
	...
	ret = FSUSER_OpenFile(&filehandle, extdata_archives[archive].archive_file, fsMakePath(PATH_ASCII, path), 1, 0);
	if(ret!=0)return ret;
...
Result archive_readfile(Archive archive, char *path, u8 *buffer, u32 size)
{
	...
	ret = FSUSER_OpenFile(&filehandle, extdata_archives[archive].archive_file, fsMakePath(PATH_ASCII, path), FS_OPEN_READ, 0);
	if(ret!=0)return ret;
...
Result archive_writefile(Archive archive, char *path, u8 *buffer, u32 size)
{
	...
	ret = FSUSER_OpenFile(&filehandle, extdata_archives[archive].archive_file, fsMakePath(PATH_ASCII, path), FS_OPEN_WRITE, 0);
	if(ret!=0)return ret;
